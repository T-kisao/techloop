"""
TechLoop — Daily News Aggregator
---------------------------------
Fetches RSS feeds, generates AI summaries via Claude Haiku,
and rebuilds index.html ready for Vercel deployment.

Usage:
    python generate.py

Requirements:
    pip install feedparser requests anthropic python-dateutil

Environment variables:
    ANTHROPIC_API_KEY  — your Anthropic API key
"""

import os
import json
import feedparser
import requests
import anthropic
from datetime import datetime, timezone
from dateutil import parser as dateparser
from pathlib import Path

# ── CONFIG ────────────────────────────────────────────────────────────────────

ANTHROPIC_API_KEY = os.environ.get("ANTHROPIC_API_KEY", "")

# Max articles to show per category on the homepage
MAX_PER_CATEGORY = 6

# Max articles to summarise with AI (keeps API costs low)
MAX_AI_SUMMARIES = 10

# How many featured articles on the hero grid
FEATURED_COUNT = 3

# ── RSS SOURCES ───────────────────────────────────────────────────────────────

FEEDS = {
    "AI": [
        "https://feeds.feedburner.com/mit-technology-review/fvDp",
        "https://www.theverge.com/ai-artificial-intelligence/rss/index.xml",
        "https://venturebeat.com/category/ai/feed/",
        "https://www.wired.com/feed/tag/artificial-intelligence/rss",
    ],
    "Gadgets": [
        "https://www.theverge.com/rss/index.xml",
        "https://9to5mac.com/feed/",
        "https://www.gsmarena.com/rss-news-reviews.php3",
        "https://www.engadget.com/rss.xml",
    ],
    "Innovation": [
        "https://news.mit.edu/rss/topic/innovation",
        "https://www.fastcompany.com/technology/rss",
        "https://feeds.newscientist.com/science-technology",
    ],
    "Startups": [
        "https://techcrunch.com/feed/",
        "https://www.siliconrepublic.com/feed/",
        "https://www.eu-startups.com/feed/",
        "https://sifted.eu/feed/",
    ],
    "Gaming": [
        "https://feeds.ign.com/ign/all",
        "https://www.eurogamer.net/feed",
        "https://www.rockpapershotgun.com/feed/",
    ],
}

# Category colours (must match index.html CSS variables)
CAT_COLORS = {
    "AI":         "cat-ai",
    "Gadgets":    "cat-gadgets",
    "Innovation": "cat-innovation",
    "Startups":   "cat-startups",
    "Gaming":     "cat-gaming",
}

# ── FETCH FEEDS ───────────────────────────────────────────────────────────────

def fetch_articles(category: str, urls: list[str]) -> list[dict]:
    """Fetch and parse RSS feeds for a category, return sorted articles."""
    articles = []
    headers = {"User-Agent": "TechLoop/1.0 (+https://techloop.ie)"}

    for url in urls:
        try:
            resp = requests.get(url, headers=headers, timeout=10)
            feed = feedparser.parse(resp.content)
            source_name = feed.feed.get("title", url.split("/")[2])

            for entry in feed.entries[:5]:  # max 5 per feed
                published = None
                if hasattr(entry, "published"):
                    try:
                        published = dateparser.parse(entry.published)
                    except Exception:
                        published = datetime.now(timezone.utc)
                else:
                    published = datetime.now(timezone.utc)

                # Ensure timezone-aware
                if published and published.tzinfo is None:
                    published = published.replace(tzinfo=timezone.utc)

                articles.append({
                    "title":     entry.get("title", "").strip(),
                    "link":      entry.get("link", ""),
                    "summary":   entry.get("summary", "")[:500].strip(),
                    "image":     _extract_image(entry),
                    "source":    source_name,
                    "category":  category,
                    "published": published,
                })
        except Exception as e:
            print(f"  ⚠ Error fetching {url}: {e}")

    # Sort by date descending, deduplicate by title
    seen = set()
    unique = []
    for a in sorted(articles, key=lambda x: x["published"] or datetime.min.replace(tzinfo=timezone.utc), reverse=True):
        key = a["title"].lower()[:60]
        if key not in seen:
            seen.add(key)
            unique.append(a)

    return unique[:MAX_PER_CATEGORY]


def _extract_image(entry) -> str:
    """Try to extract a thumbnail image from a feed entry."""
    # media:thumbnail
    if hasattr(entry, "media_thumbnail") and entry.media_thumbnail:
        return entry.media_thumbnail[0].get("url", "")
    # media:content
    if hasattr(entry, "media_content") and entry.media_content:
        for m in entry.media_content:
            if m.get("type", "").startswith("image"):
                return m.get("url", "")
    # enclosures
    if hasattr(entry, "enclosures") and entry.enclosures:
        for enc in entry.enclosures:
            if enc.get("type", "").startswith("image"):
                return enc.get("href", "")
    return ""

# ── AI SUMMARIES ──────────────────────────────────────────────────────────────

def generate_summary(client: anthropic.Anthropic, article: dict) -> str:
    """Generate a 2-sentence AI summary for an article using Claude Haiku."""
    prompt = (
        f"Write a 2-sentence summary of this tech article for a general audience. "
        f"Be concise, factual, and engaging. Do not start with 'This article'.\n\n"
        f"Title: {article['title']}\n"
        f"Content: {article['summary']}"
    )
    try:
        message = client.messages.create(
            model="claude-haiku-4-5-20251001",
            max_tokens=120,
            messages=[{"role": "user", "content": prompt}],
        )
        return message.content[0].text.strip()
    except Exception as e:
        print(f"  ⚠ AI summary failed for '{article['title'][:40]}': {e}")
        return article["summary"][:200] + "…"


def generate_daily_digest(client: anthropic.Anthropic, all_articles: list[dict]) -> str:
    """Generate the AI Daily Digest paragraph from today's top stories."""
    top = all_articles[:8]
    titles = "\n".join(f"- [{a['category']}] {a['title']}" for a in top)
    prompt = (
        "You are the editor of TechLoop, a tech news digest. "
        "Write a single engaging paragraph (4-5 sentences) summarising today's most important tech stories. "
        "Highlight 3-4 specific stories by name. Be sharp, insightful, and written for a European tech audience. "
        "Do not use bullet points.\n\n"
        f"Today's top stories:\n{titles}"
    )
    try:
        message = client.messages.create(
            model="claude-haiku-4-5-20251001",
            max_tokens=200,
            messages=[{"role": "user", "content": prompt}],
        )
        return message.content[0].text.strip()
    except Exception as e:
        print(f"  ⚠ Digest generation failed: {e}")
        return "Today's digest could not be generated. Check back soon."

# ── HTML GENERATION ───────────────────────────────────────────────────────────

def time_ago(dt: datetime) -> str:
    """Return human-friendly relative time string."""
    if not dt:
        return ""
    now = datetime.now(timezone.utc)
    diff = now - dt
    minutes = int(diff.total_seconds() / 60)
    if minutes < 60:
        return f"{minutes}m ago"
    hours = minutes // 60
    if hours < 24:
        return f"{hours}h ago"
    days = hours // 24
    return f"{days}d ago"


def article_card_html(article: dict, size: str = "normal") -> str:
    """Render an article as an HTML card."""
    cat_class = CAT_COLORS.get(article["category"], "cat-ai")
    ago       = time_ago(article["published"])
    title_esc = article["title"].replace('"', "&quot;")
    summary   = article.get("ai_summary") or article["summary"][:180] + "…"

    # Image or placeholder
    if article.get("image"):
        img_html = f'<img src="{article["image"]}" alt="{title_esc}" style="width:100%;height:160px;object-fit:cover;border-radius:8px;margin-bottom:1.2rem">'
    else:
        img_html = (
            '<div class="card-image-placeholder">'
            '<div class="img-pattern"></div>'
            '</div>'
        ) if size == "main" else ""

    badge = '<div class="ai-badge">⚡ AI Summary</div>' if article.get("ai_summary") else ""

    if size == "main":
        return f"""
        <a href="{article['link']}" target="_blank" rel="noopener" class="card card-main" style="text-decoration:none">
          <span class="card-cat {cat_class}">{article['category']}</span>
          {img_html}
          {badge}
          <h2 class="card-title" style="font-size:1.5rem;margin-bottom:1rem">{article['title']}</h2>
          <p class="card-summary">{summary}</p>
          <div class="card-meta">
            <span class="card-source">{article['source']}</span>
            <span>·</span><span>{ago}</span>
          </div>
        </a>"""
    else:
        return f"""
        <a href="{article['link']}" target="_blank" rel="noopener" class="card" style="text-decoration:none">
          <span class="card-cat {cat_class}">{article['category']}</span>
          {badge}
          <h3 class="card-title">{article['title']}</h3>
          <p class="card-summary">{summary}</p>
          <div class="card-meta">
            <span class="card-source">{article['source']}</span>
            <span>·</span><span>{ago}</span>
          </div>
        </a>"""


def latest_card_html(article: dict) -> str:
    """Render a compact article card for the Latest section."""
    cat_class = CAT_COLORS.get(article["category"], "cat-ai")
    ago       = time_ago(article["published"])
    summary   = article.get("ai_summary") or article["summary"][:160] + "…"
    return f"""
        <a href="{article['link']}" target="_blank" rel="noopener" class="article-card" style="text-decoration:none">
          <span class="card-cat {cat_class}">{article['category']}</span>
          <h3 class="card-title">{article['title']}</h3>
          <p class="card-summary">{summary}</p>
          <div class="card-meta">
            <span class="card-source">{article['source']}</span>
            <span>·</span><span>{ago}</span>
          </div>
        </a>"""


def ticker_items_html(articles: list[dict]) -> str:
    """Build ticker HTML from latest articles (duplicated for infinite scroll)."""
    cat_class_map = {
        "AI": "tc-ai", "Gadgets": "tc-gadgets",
        "Innovation": "tc-innovation", "Startups": "tc-startups", "Gaming": "tc-gaming",
    }
    items = ""
    for a in articles[:12]:
        tc = cat_class_map.get(a["category"], "tc-ai")
        items += f'<span class="ticker-item"><span class="ticker-cat {tc}">{a["category"]}</span> {a["title"]}</span>\n    '
    return items + items  # duplicate for seamless loop


def category_counts_html(all_articles: list[dict]) -> str:
    """Generate article counts per category for the Browse section."""
    counts = {}
    for a in all_articles:
        counts[a["category"]] = counts.get(a["category"], 0) + 1

    icons = {"AI": "🧠", "Gadgets": "📱", "Innovation": "💡", "Startups": "🚀", "Gaming": "🎮"}
    html  = ""
    for cat, cls in CAT_COLORS.items():
        count = counts.get(cat, 0)
        icon  = icons.get(cat, "📰")
        html += f"""
      <a href="/category/{cat.lower()}" class="cat-card cat-{cat.lower()}-card" style="display:block;text-decoration:none">
        <div class="cat-icon">{icon}</div>
        <div class="cat-name">{cat}</div>
        <div class="cat-count">{count} articles</div>
      </a>"""
    return html

# ── TEMPLATE REBUILD ──────────────────────────────────────────────────────────

def rebuild_html(all_articles: list[dict], digest: str) -> None:
    """Read index.html template, inject live content, write output."""
    template_path = Path("index.html")
    if not template_path.exists():
        print("❌ index.html not found. Place generate.py next to index.html.")
        return

    html = template_path.read_text(encoding="utf-8")
    now  = datetime.now(timezone.utc)
    timestamp = now.strftime("%-d %b %Y · %H:%M UTC")

    # Featured grid (top 3 articles across all categories)
    featured = all_articles[:FEATURED_COUNT]
    featured_html = ""
    for i, article in enumerate(featured):
        size = "main" if i == 0 else "normal"
        featured_html += article_card_html(article, size)

    # Latest section (next 3 after featured)
    latest = all_articles[FEATURED_COUNT:FEATURED_COUNT + 3]
    latest_html = "".join(latest_card_html(a) for a in latest)

    # Ticker
    ticker_html = ticker_items_html(all_articles)

    # Category counts
    cats_html = category_counts_html(all_articles)

    # Digest paragraph — bold key phrases
    digest_sentences = digest.split(". ")
    digest_formatted = ". ".join(digest_sentences)

    # Inject into HTML using comment markers
    replacements = {
        "<!-- INJECT:FEATURED -->":   featured_html,
        "<!-- INJECT:LATEST -->":     latest_html,
        "<!-- INJECT:TICKER -->":     ticker_html,
        "<!-- INJECT:CATS -->":       cats_html,
        "<!-- INJECT:DIGEST -->":     digest_formatted,
        "<!-- INJECT:TIMESTAMP -->":  timestamp,
    }
    for marker, content in replacements.items():
        html = html.replace(marker, content)

    output_path = Path("index.html")
    output_path.write_text(html, encoding="utf-8")
    print(f"✅ index.html rebuilt — {len(all_articles)} articles injected.")

# ── MAIN ──────────────────────────────────────────────────────────────────────

def main():
    print("🔄 TechLoop — starting daily build\n")

    if not ANTHROPIC_API_KEY:
        print("⚠ ANTHROPIC_API_KEY not set — AI summaries will be skipped.\n")
        client = None
    else:
        client = anthropic.Anthropic(api_key=ANTHROPIC_API_KEY)

    # 1. Fetch all feeds
    all_articles = []
    for category, urls in FEEDS.items():
        print(f"📡 Fetching {category}...")
        articles = fetch_articles(category, urls)
        print(f"   {len(articles)} articles fetched")
        all_articles.extend(articles)

    # Sort all by date
    all_articles.sort(
        key=lambda x: x["published"] or datetime.min.replace(tzinfo=timezone.utc),
        reverse=True
    )

    print(f"\n📰 Total articles: {len(all_articles)}")

    # 2. Generate AI summaries for top articles
    if client:
        print(f"\n🤖 Generating AI summaries (top {MAX_AI_SUMMARIES})...")
        for i, article in enumerate(all_articles[:MAX_AI_SUMMARIES]):
            print(f"   [{i+1}/{MAX_AI_SUMMARIES}] {article['title'][:50]}...")
            article["ai_summary"] = generate_summary(client, article)

        # 3. Generate daily digest
        print("\n✦  Generating AI Daily Digest...")
        digest = generate_daily_digest(client, all_articles)
        print(f"   Done: {digest[:80]}...")
    else:
        digest = "Today's AI digest is unavailable — ANTHROPIC_API_KEY not configured."

    # 4. Rebuild HTML
    print("\n🔨 Rebuilding index.html...")
    rebuild_html(all_articles, digest)

    print("\n🚀 Build complete. Deploy index.html to Vercel.")


if __name__ == "__main__":
    main()
